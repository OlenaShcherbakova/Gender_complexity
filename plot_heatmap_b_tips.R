...
p1 <- gheatmap(plain_tree, gb.subset[, 2:3], offset=8, width=.9, colnames_angle=0, 
               colnames_offset_y = .5, font.size=5, hjust=0.5, color = NULL, colnames_position = "top") + 
  #ylim(-3, 85) +
  scale_fill_viridis_c(option="D", name="continuous values:", direction=-1, limits=c(0, 1))

p2 <- p1 + new_scale_fill()

p2 <- gheatmap(p2, gb.subset[, 4:5], offset=22, width=.9, colnames_angle=0, 
               colnames_offset_y = .5, font.size=5, hjust=0.5, color = NULL, colnames_position = "top") + 
  #ylim(-3, 85) +
...
ggsave(file="output/heatmap_b_rect.svg", plot=p2, width=16, height=16)
