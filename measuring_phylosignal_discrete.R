...
physig_phon_prop_D <-
  caper::phylo.d(data = data,
                 binvar = phon_prop,
                 permut = 1000)
physig_phon_prop_DEstimate <-
  physig_phon_prop_D$DEstimate[1][["Obs"]]
...
phon_prop_ie <-
  c(
    physig_phon_prop_DEstimate,
    physig_phon_prop_Pval1,
    physig_phon_prop_Pval0,
    "Indo-European"
  )

physig_unpredictable_D <-
  caper::phylo.d(data = data,
                 binvar = unpredictable,
                 permut = 1000)
physig_unpredictable_DEstimate <-
  physig_unpredictable_D$DEstimate[1][["Obs"]]
physig_unpredictable_Pval1 <-
  physig_unpredictable_D$Pval1 # D compared to 1
physig_unpredictable_Pval0 <-
  physig_unpredictable_D$Pval0 # D compared to 0
unpredictable_ie <-
  c(
    physig_unpredictable_DEstimate,
    physig_unpredictable_Pval1,
...
physig <- physig %>%
  ...
  mutate_at(vars(-V2,-V3), 
            ~ ifelse(is.numeric(.), round(., digits = 2), .)) %>%
  rename(`D value` = V1) %>%
  rename(`p-value: departure from phylogenetic randomness` = V2) %>%
  rename(`p-value: departure from Brownian threshold model` = V3) %>%
  mutate(`p-value: departure from phylogenetic randomness` = case_when(
    `p-value: departure from phylogenetic randomness` < 0.001 ~ "<0.001",
    `p-value: departure from phylogenetic randomness` < 0.01 ~ "<0.01",
    TRUE ~ as.character(round(`p-value: departure from phylogenetic randomness`,2))
  )) %>% 
  mutate(`p-value: departure from Brownian threshold model` = case_when(
    `p-value: departure from Brownian threshold model` < 0.001 ~ "<0.001",
    `p-value: departure from Brownian threshold model` < 0.01 ~ "<0.01",
    TRUE ~ as.character(round(`p-value: departure from Brownian threshold model`,2))
  )) %>% 
  rename(`Phylogeny` = V4) %>%
  relocate(Phylogeny, .before = 1) %>%
  add_column(Feature = rep(c("Phonological rules", "Unpredictable"), times =
                             3), .after = "Phylogeny")

rownames(physig) <- NULL

write.csv(physig,
          "output_tables/Phylogenetic_signal_D_value.csv",
          row.names = FALSE)
