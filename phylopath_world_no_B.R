...
to_exclude <- grambank_phylopath_compl$Glottocode
...
grambank_phylopath_compl <- grambank_phylopath_compl %>%
  filter(sem_classes != "0" | agr_patterns != "0") %>% 
  filter(!Glottocode %in% to_exclude)
...
write.csv(table_final, file="output_tables/Phylopath_output_table_W_no_B.csv")
...
save_as_docx(
  "Phylogenetic path analysis (World without Bantu languages)" = table_final, 
  path = "output_tables/Phylopath_output_table_W_no_B.docx")
...
table_final_latex <- table %>%
  ...
  mutate(Phylogeny = "World") %>%
  relocate(Phylogeny, .after = Model) %>%
  kbl(caption="Phylogenetic path analysis (World without Bantu languages)",
      format="latex") %>% #,
...
save(b_ci, 
     file = "output/phylopath_w_no_B.RData")
load('output/phylopath_w_no_B.RData')
