...
table_final <- table %>% 
  ...
  mutate(Phylogeny = "Indo-European") %>%
  relocate(Phylogeny, .after = Model)

write.csv(table_final, file="output_tables/Phylopath_output_table_IE.csv")
...
save_as_docx(
  "Phylogenetic path analysis (Indo-European)" = table_final, 
  path = "output_tables/Phylopath_output_table_IE.docx")
...
table_final_latex <- table %>%
  ...
  mutate(Phylogeny = "Indo-European") %>%
  relocate(Phylogeny, .after = Model) %>%
  kbl(caption="Phylogenetic path analysis (Indo-European)",
      format="latex") %>% #,
...
save(b_ci, 
     file = "output/phylopath_ie.RData")
